...
@section('content')
    <a style="float:right" href="{{ route('logout') }}" onclick="event.preventDefault();
                ...
                <div class="col-12">
                    <div class="card m-5">
                        <div class="card-header">
                            ...
                            <h4 class="card-title" style="float:right">Total Active User Is : {{ $activeuser }}</h4>
                            <br>
                            <h4 class="card-title" style="float:right">Total InActive User Is : {{ $inactiveuser }}
                            </h4>
                            ...
                            <table class="table table-bordered table-hover">
                                ...
                                <tbody>
                                    @foreach ($users as $user)
                                        <tr class="todo-{{ $user['id'] }}" id="{{ $user->id }}"
                                            ...
                                            <td>
                                                @if ($user->status === 'Active')
                                                    <span style="color:green;"
                                                        id="status_{{ $user->id }}">Active</span>
                                                @else
                                                    <span style="color:red;"
                                                        id="status_{{ $user->id }}">InActive</span>
                                                @endif
                                            </td>

                                            <td>
                                                @if ($user->status === 'Active')
                                                    <input type="checkbox" id="ChkStatus_{{ $user->id }}" name="status"
                                                        checked class="chkStatus">
                                                @else
                                                    <input type="checkbox" id="ChkStatus_{{ $user->id }}" name="status"
                                                        class="chkStatus">
                                                @endif
                                            ...
                                            <td>
                                                ...
                                                <a style="margin-top: 5px;" href="#"
                                                    class="fas fa-trash-alt delete_btn"></a>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
